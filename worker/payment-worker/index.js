...
async function handleQuote(request, env) {
  ...
  if (!clientId || typeof clientId !== 'string' || clientId.length < 16) return json({ error: 'clientId required (min 16 chars)' }, 400);
  ...
  return json({
    payment_id: paymentId, chain: methodConfig.chain, token: methodConfig.token,
    address, amount: Number(finalRaw) / Math.pow(10, rate.decimals),
    amount_raw: finalRaw.toString(), decimals: rate.decimals,
    usd_price: usdAmount, credits: bundleConfig.credits, expires_at: expiresAt,
  });
}
...
async function handleVerify(request, env) {
  ...
  if (pending.verified) return json({ ok: true, alreadyVerified: true, credits: pending.credits });
  if (new Date(pending.expiresAt) < new Date()) return json({ error: 'payment expired' }, 410);
  const found = await scanForPayment(env, pending);
  if (!found) return json({ status: 'pending', message: 'No matching transaction found yet' });
  ...
  await env.PENDING_PAYMENTS.put(`crypto:${payment_id}`, JSON.stringify(pending), { expirationTtl: 60 * 60 * 24 * 7 });
  return json({ ok: true, creditsAdded: pending.credits, newBalance: balance, txHash: found.txHash });
}
...
async function handleFeeRecord(request, env) {
  ...
  if (!clientId) return json({ error: 'clientId required' }, 400);
  if (!Array.isArray(receipts) || receipts.length === 0) return json({ error: 'receipts required (non-empty array)' }, 400);

  const sweepId = crypto.randomUUID();
  const record = {
    sweepId, clientId, receipts,
...
async function handleFeePending(request, env) {
  ...
  return json({ ok: true, count: items.length, totalPending: pendingIds.length, items });
}
...
async function handleFeeMarkForwarded(request, env) {
  ...
  if (record.status === 'forwarded') return json({ ok: true, alreadyForwarded: true, record });
  ...
  await env.CREDITS.put(`fee:forwarded:${sweepId}`, JSON.stringify(record), { expirationTtl: 60 * 60 * 24 * 365 });
  const pendingRaw = await env.CREDITS.get('fee:pending');
  const pending = pendingRaw ? JSON.parse(pendingRaw) : [];
...
async function handleFeeSummary(request, env) {
  ...
  for (const [key, v] of Object.entries(totalsByChain)) {
    totals[key] = {
      ...
      amountRaw: v.raw.toString(), count: v.count,
    };
  }
  return json({ ok: true, totalPendingSweeps, totals });
}
