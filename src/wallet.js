...
export class WalletConnectBackend {
  ...
  async getEthersSigner(_provider) { return this.provider.getSigner(); }
  getSolanaKeypair() { throw new Error('WalletConnect does not support Solana'); }
  getBitcoinKeyPair() { throw new Error('WalletConnect does not support Bitcoin'); }
  async dispose() {
    try { await this.provider.disconnect?.(); } catch {}
  }
}

export async function connectWalletConnect({ projectId, chains = [1, 42161, 10, 8453, 137], onUri }) {
  ...
  return new WalletConnectBackend(ethersProvider, accounts[0], wcProvider.chainId);
}
...
export class LedgerBackend {
  ...
  async getEthersSigner(provider) {
    ...
    class LedgerSigner extends ethers.AbstractSigner {
      constructor(provider) { super(provider); this.address = address; }
      async getAddress() { return this.address; }
      connect(provider) { return new LedgerSigner(provider); }
      async signTransaction(tx) {
        const unsignedTx = ethers.Transaction.from(tx);
        const sig = await ethApp.signTransaction(path, unsignedTx.unsignedSerialized.slice(2));
        unsignedTx.signature = ethers.Signature.from({
          r: '0x' + sig.r, s: '0x' + sig.s, v: parseInt(sig.v, 16),
        });
        return unsignedTx.serialized;
      }
      async signMessage(message) {
        const hex = typeof message === 'string' ? Buffer.from(message).toString('hex') : Buffer.from(message).toString('hex');
        const sig = await ethApp.signPersonalMessage(path, hex);
  ...
  getSolanaKeypair() { throw new Error('Ledger Solana not supported'); }
  getBitcoinKeyPair() { throw new Error('Ledger Bitcoin not supported'); }
  async dispose() { try { await this.transport.close(); } catch {} }
}

export async function connectLedger({ derivationPath = "44'/60'/0'/0/0" } = {}) {
  if (!navigator.hid) throw new Error('WebHID not supported. Use Chrome or Edge.');
...
export class TrezorBackend {
  ...
  async getEthersSigner(provider) {
    ...
    class TrezorSigner extends ethers.AbstractSigner {
      constructor(provider) { super(provider); this.address = address; }
      async getAddress() { return this.address; }
      connect(provider) { return new TrezorSigner(provider); }
      async signTransaction(tx) {
        ...
        const unsignedTx = ethers.Transaction.from(tx);
        unsignedTx.signature = ethers.Signature.from({ r, s, v: parseInt(v, 16) });
        return unsignedTx.serialized;
      }
      async signMessage(message) {
        const { default: TrezorConnect } = await import('@trezor/connect-web');
        const hex = typeof message === 'string' ? Buffer.from(message).toString('hex') : Buffer.from(message).toString('hex');
        const result = await TrezorConnect.ethereumSignMessage({ path, message: hex, hex: true });
        if (!result.success) throw new Error(result.payload.error);
        const { v, r, s } = result.payload;
  ...
  getSolanaKeypair() { throw new Error('Trezor Solana not supported'); }
  getBitcoinKeyPair() { throw new Error('Trezor Bitcoin not supported'); }
  async dispose() {}
}
...
export async function connectTrezor({ derivationPath = "m/44'/60'/0'/0/0" } = {}) {
  ...
  await TrezorConnect.init({
    lazyLoad: true,
    manifest: { email: 'hello@example.com', appUrl: window.location.origin },
  });
  const result = await TrezorConnect.ethereumGetAddress({ path: derivationPath, showOnTrezor: true });
  if (!result.success) throw new Error(result.payload.error);
...
export async function createWallet(config) {
  switch (config.type) {
    ...
    case 'walletconnect':
      return await connectWalletConnect({ projectId: config.wcProjectId, onUri: config.wcOnUri });
    case 'ledger':
      return await connectLedger({ derivationPath: config.derivationPath });
    case 'trezor':
      return await connectTrezor({ derivationPath: config.derivationPath });
    default:
